...
for n = 1:Nt
    ...
    % Fill in the A matrix for the implicit scheme
    for i = 2:Nx
    ...
    % Solve the linear system using BiCGStab iterative solver
    [u_implicit, ~] = bicgstab(A, B, 1e-6, 1000);
...
% Display the time taken for the wave to leave the domain
fprintf('Time taken for the wave to leave the domain (implicit): %.2f seconds\n', time_wave_leaves_implicit);

% Plot the final wave position
figure;
plot(x, u_implicit, 'LineWidth', 2);
xlabel('Spatial coordinate');
ylabel('Wave amplitude');
title('Final Wave Position Using Implicit Upwind Scheme');
grid on;
